...
- Name: constructor.test.tsx >> Burger constructor >> Добавление ингредиента из списка ингредиентов в конструктор
- Location: src\tests\constructor.test.tsx:90:7
...
```ts
  1   | // tests/constructor.pl.tsx
  2   | import { test, expect } from '@playwright/test';
  3   | import fs from 'node:fs';
  4   | import path from 'node:path';
  5   | 
  6   | const HAR_DIR = path.resolve(__dirname, '../tests/hars');
  7   | const INGREDIENTS_HAR = path.join(HAR_DIR, 'ingredients.har');
  8   | const ORDERS_HAR = path.join(HAR_DIR, 'orders.har');
  9   | 
